...
import { apiErrorNotice } from '../actions.js';
...
export async function replyNearbyStatus(ctx, lat, lng, label) {
    ...
    if (!ok) {
        await ctx.reply(apiErrorNotice(error).fallback, Markup.removeKeyboard());
        return;
    }

    const active = alerts.filter(a => a.msgType !== 'Cancel');
    const rich = formatActiveAlertsList(active, lat, lng, { title: label });
    ...
    await sendRichMessage(ctx.telegram, ctx.chat.id, rich, null, Markup.removeKeyboard());
