// Service worker registration and the update bar. Loaded once per page as a
// module; nothing else registers the worker.
//
// A new worker never activates on its own (see sw.js). When one has
// downloaded and is waiting, this draws a slim bar at the top of the page
// offering Reload and Not now. Nothing reloads until the reader asks.

const SW_URL = '/sw.js';

const STRINGS = {
  label: 'Update',
  ready: 'A new version of SG Flood Watch is ready.',
  reload: 'Reload',
  later: 'Not now',
};
...
function registerWorker() {
  ...
  // new worker and not the one being replaced. The flag stops a second
...
function render() {
  ...
  // status, not alert: nothing is wrong.
  ...
  bar.innerHTML = `
    <div class="update-notice-inner">
      ...
      <button type="button" class="btn btn-secondary" data-sw-later>${STRINGS.later}</button>
    </div>
  `;
